...
import { collection, query, where, getDocs, addDoc, doc, getDoc, onSnapshot, setDoc } from "firebase/firestore";
                ...
                <div className="bg-white h-[43rem] w-72 rounded-2xl pb-5">
                    ...
                    {chatUsers.length > 0 ? (
                        chatUsers.map((user) => {
                            ...
                            return <Card key={user.id} name={user.chatPartnerName} unreadCount={unreadCount[user.chatId]} onClick={() => handleChatSelection(user)} />;
                        })
                    ) : (
